...
public class TaskUtils {
    ...
    public static TaskStatus getTaskStatus(CropScheduleTask task, LocalDateTime taskDate) {
        LocalDateTime now = LocalDateTime.now();

        //todo check if task is completed after implementing inheritance

        if (task.getTaskStatus().equals(TaskStatus.IN_PROGRESS) && task.getIsCompleted()) {
